...
// $('#example').barrating({
      ...
      //});
        	...
        	var tmpl = $.templates('<div class="col-xs-12 col-sm-6 col-md-4 col-lg-3"><div class="thumbnail"><div class="img-container" id="img-hover"><img src="{{:pictureUrl}}" alt="..."><span class="quick-view">Quick view</span></div><div class="caption"><h3 class="goods-name">{{:name}}</h3><p class="price">{{:price}}$</p><select class="rating"><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option></select><div class="btn-group" role="group" aria-label="..."><button type="button" class="btn btn-default add-cart"><i class="glyphicon glyphicon-shopping-cart"></i></button><button type="button" class="btn btn-default"><i class="glyphicon glyphicon-retweet"></i></button><button type="button" class="btn btn-default"><i class="glyphicon glyphicon-heart"></i></button></div></div></div></div></div>');
			...
			data.forEach(function(bonsai) {
        		...
        		var ratingElement = $(bonsaiHtml).find(".rating").first();
...
function sendMess(e) {
    e.preventDefault();
    $('.ok-icon').css({'visibility' : 'visible'});
    setTimeout(function() {
        $('.ok-icon').css({'visibility' : 'hidden'});
    }, 3000);
}
